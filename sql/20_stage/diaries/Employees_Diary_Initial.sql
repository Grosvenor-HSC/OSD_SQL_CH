...
BEGIN
    ...
    BEGIN TRY
        ...
        --------------------------------------------------------------------
        -- 2) Fence CT window at START (incremental will pick up later changes)
        --------------------------------------------------------------------
        ...
        --------------------------------------------------------------------
        -- 4) Recreate target (Employee_UUID = INT; PK (Employee_UUID, UUID))
        --------------------------------------------------------------------
        ...
        (
            Employee_UUID  INT           NOT NULL,  -- EMPLOYEE.EMP_REF
            UUID           INT           NOT NULL,  -- EMP_DY_REF
        ...
        DECLARE @sql nvarchar(max) = N'
            ...
            (
                Employee_UUID, UUID, Entry_Date, Review_Date, Entry_Type,
        ...
        -- Optional:
        -- CREATE NONCLUSTERED INDEX IX_tbl_EmployeesDiary_EntryType
        --     ON dbo.tbl_EmployeesDiary (Entry_Type);
        ...
        --------------------------------------------------------------------
        -- 8) Quiet incremental top-off (robust to old/new signatures)
        --------------------------------------------------------------------
        ...
        BEGIN
            BEGIN TRY
                -- Try NEW signature (with @Summary OUTPUT + @ReturnSummaryRow)
                SET @IncrMsg = N'';
            ...
            END TRY
            BEGIN CATCH
                -- Fallback to OLD signature if new one isn't available
                ...
                BEGIN CATCH
                    ...
                    SET @IncrMsg = CONCAT(N'EmployeesDiary incremental failed to run: ', @em);
                END CATCH
            END CATCH
        END

        -- Return exactly two rows (Initial + Incremental)
